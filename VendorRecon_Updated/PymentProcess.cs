...
{
    public class PymentProcess : PXGraph<PymentProcess>
    {


        public PXCancel<Filters> Cancel;
        public PXFilter<Filters> MasterView;

        [PXFilterable]
        public PXFilteredProcessing<VendorRecon, Filters, Where2<
                Where<VendorRecon.paymentStatus, Equal<Filters.open>, And<Filters.vendorID.FromCurrent, IsNull>>,
                 Or<Where<VendorRecon.paymentStatus, Equal<Filters.open>, And<VendorRecon.vendorID, Equal<Filters.vendorID.FromCurrent>>>>>, OrderBy<Asc<VendorRecon.periodID>>> DetailsView;


        private static string billNbr;

        public PymentProcess()
        {
            DetailsView.SetSelected<VendorRecon.selected>();
        }
        ...
        public static string displayRefNbr { get; private set; }

        public static void ProcessPayment(List<VendorRecon> list, VendorReconEntry vendorReconEntry, bool isMassProcess = false)
        {
            ...
            if (isMassProcess)
            {
                ...
                if (count == 0)
                {
                    PXProcessing.SetProcessed<VendorRecon>();
                    throw new PXRedirectRequiredException(aPPrintChecks, true, "Processing");
                }
            }
        }

        public static void Process(List<VendorRecon> list)
        {
            var pendingPrint = false;
        ...
        protected virtual void _(Events.RowSelected<VendorRecon> e)
        {
            //DetailsView.SetProcessWorkflowAction<VendorReconEntry>(
            //g => g.ReadyForPayment);

            DetailsView.SetProcessDelegate(
                delegate (List<VendorRecon> list)
                {
